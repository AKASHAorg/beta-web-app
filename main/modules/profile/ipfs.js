    ...
    while (i < simpleLinks.length) {
        if (!isEmpty(data[simpleLinks[i]]) && data[simpleLinks[i]]) {
            tmp = yield IpfsConnector.getInstance()
                .api
                .add(data[simpleLinks[i]], simpleLinks[i] === ProfileSchema.AVATAR, (simpleLinks[i] === ProfileSchema.AVATAR) && is(String, data[simpleLinks[i]]));
...
export const getShortProfile = Promise.coroutine(function* (hash, resolveAvatar) {
...
export const resolveProfile = Promise.coroutine(function* (hash, resolveImages) {
