...
import { channel as Channel, web3Api } from 'services';
...
function* createEthAddressRequest (tempProfile) {
    const channel = Channel.instance.server.auth.generateEthKey;
...
function* createEthAddressListener (tempProfile) {
    const tempProfileStatus = yield select(state => state.tempProfileState.get('status'));
        tempProfile.address = web3Api.instance.eth.accounts[0];
...
function* requestEther (tempProfile) {
    const channel = Channel.instance.server.auth.requestEther;
    yield put(tempProfileActions.faucetRequest(tempProfile));
    yield call([channel, channel.send], { address:  web3Api.instance.eth.accounts[0] });
}
/**
...
function* addTxToQueue (tx) {
    const channel = Channel.instance.server.tx.addToQueue;
...
function* tempProfilePublishRequest (tempProfile) {
    const channel = Channel.instance.server.registry.registerProfile;
    const manager = Channel.instance.client.registry.manager;
