...
export function* profileGetCurrent () {
    const channel = Channel.instance.server.registry.getCurrentProfile;
    yield apply(channel, channel.send, [{}]);
}

function* profileGetData (profile, full = false) {
    const channel = Channel.instance.server.profile.getProfileData;
    yield apply(channel, channel.send, [{ profile, full }]);
}

function* profileGetList ({ profileAddresses }) {
    const channel = Channel.instance.server.profile.getProfileList;
    yield call(enableChannel, channel, Channel.instance.client.profile.manager);
    yield apply(channel, channel.send, [profileAddresses]);
}

function* profileGetLocal () {
    const channel = Channel.instance.server.auth.getLocalIdentities;
    yield call(enableChannel, channel, Channel.instance.client.auth.manager);
    yield apply(channel, channel.send, [{}]);
}
...
function* profileIsFollower ({ followings, akashaId }) {
    const channel = Channel.instance.server.profile.isFollower;
    yield call(enableChannel, channel, Channel.instance.client.profile.manager);
    if (!akashaId) {
...
function* profileLogin ({ data }) {
    ...
    const { ...payload } = data;
    const channel = Channel.instance.server.auth.login;
    payload.password = new global.TextEncoder('utf-8').encode(payload.password);
    yield apply(channel, channel.send, [payload]);
}

function* profileLogout () {
    const channel = Channel.instance.server.auth.logout;
    yield apply(channel, channel.send, [{}]);
}
...
function* profileSendTip ({ akashaId, receiver, value, gas }) {
    const channel = Channel.instance.server.profile.tip;
    yield call(enableChannel, channel, Channel.instance.client.profile.manager);
    const token = yield select(selectToken);
...
function* profileUnfollow ({ akashaId, gas, profile }) {
    const channel = Channel.instance.server.profile.unFollowProfile;
    yield call(enableChannel, channel, Channel.instance.client.profile.manager);
    const token = yield select(selectToken);
    yield apply(channel, channel.send, [{ token, akashaId, gas, profile }]);
}
