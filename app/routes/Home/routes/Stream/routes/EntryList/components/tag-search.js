...
class TagSearch extends Component {
    constructor (props) {
        ...
        channel.instance.client.tags.searchTag.on(this.hydrateDataSource);
        channel.instance.client.tags.exists.on(this.checkTag);
    }

    componentWillUnmount () {
        channel.instance.client.tags.searchTag.removeListener(this.hydrateDataSource);
        channel.instance.client.tags.exists.removeListener(this.checkTag);
    }

    componentWillReceiveProps (nextProps) {
        if (this.props.tagName !== nextProps.tagName || this.state.pendingTag) {
            channel.instance.server.tags.exists.send({ tagName: nextProps.tagName });
        }
    }
    ...
    delayedReq = debounce(() => {
        if (this.state.typed.length >= 3) {
            channel.instance.server.tags.searchTag.send({ tagName: this.state.typed, limit: 5 });
        }
    }, 210);
