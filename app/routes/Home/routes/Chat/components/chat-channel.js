...
class ChatChannel extends Component {
    ...
    componentDidMount () {
        ...
        channel.instance.client.chat.fetch.on(this.hydrateMessages);
        channel.instance.server.chat.fetch.send({ channel: params.channel });
    ...
    componentWillUnmount () {
        channel.instance.client.chat.fetch.removeListener(this.hydrateMessages);
        if (this.timeout) {
            clearTimeout(this.timeout);
        }
    ...
    handleKeyDown = (ev) => {
        if (ev.key === 'Enter') {
            ev.preventDefault();
            channel.instance.server.chat.post.send({ message: ev.target.value });
            this.setState({
                currentMessage: ''
            });
        }
