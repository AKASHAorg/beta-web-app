import { defineMessages } from 'react-intl';

// keep this sorted alphabetically
const placeholderMessages = defineMessages({
    akasha: {
        id: 'app.placeholder.akasha',
        description: 'footer subtitle',
        defaultMessage: 'Akasha'
    },
    contacts: {
        id: 'app.placeholder.contacts',
        description: 'footer subtitle',
        defaultMessage: 'Contacts'
    },
    downloadDesktopApp: {
        id: 'app.placeholder.downloadDesktopApp',
        description: 'title for web placeholder card',
        defaultMessage: 'Download Desktop App'
    },
    downloadDesktopAppSubtitle: {
        id: 'app.placeholder.downloadDesktopAppSubtitle',
        description: 'subtitle for web placeholder card',
        defaultMessage: 'For the full experience try our desktop application'
    },
    welcome: {
        id: 'app.placeholder.welcome',
        description: 'title for web placeholder page',
        defaultMessage: 'Welcome to AKASHA'
    },
    welcomeSubtitle: {
        id: 'app.placeholder.welcomeSubtitle',
        description: 'subtitle for web placeholder page',
        defaultMessage: 'Decentralised Social Media Platform based on Ethereum and IPFS'
    },
    tryBrowser: {
        id: 'app.placeholder.tryBrowser',
        description: 'title for web placeholder card',
        defaultMessage: 'Try Akasha on browser'
    },
    tryBrowserSubtitle: {
        id: 'app.placeholder.tryBrowserSubtitle',
        description: 'subtitle for web placeholder card',
        defaultMessage: 'Fast and easy to use'
    },
    getExtension: {
        id: 'app.placeholder.getExtension',
        description: 'button title',
        defaultMessage: 'Get Metamask'
    },
    getBrave: {
        id: 'app.placeholder.getBrave',
        description: 'button title',
        defaultMessage: 'Get Brave Browser'
    },
    download: {
        id: 'app.placeholder.download',
        description: 'button title',
        defaultMessage: 'Download'
    },
    terms: {
        id: 'app.placeholder.terms',
        description: 'placeholder card info',
        defaultMessage: 'By downloading, you agree to the {termsLink}'
    },
    termsOfService: {
        id: 'app.placeholder.termsOfService',
        description: 'terms of service title',
        defaultMessage: 'Terms of Service'
    },
    blog: {
        id: 'app.placeholder.blog',
        description: 'blog subfooter',
        defaultMessage: 'Blog'
    },
    troubleConnecting: {
        id: 'app.placeholder.troubleConnecting',
        description: 'title for geth error card',
        defaultMessage: 'No extension on the test network'
    },
    wrongNetwork: {
        id: 'app.placeholder.wrongNetwork',
        description: 'title for geth error card',
        defaultMessage: 'Please set Metamask to run on the Rinkeby test network.'
    }
});
export { placeholderMessages };
